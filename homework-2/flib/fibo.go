...
//FiboNumberSlow Тупая регрессия (работает адски медленно)
func FiboNumberSlow(n int) (out int64) {
	if n == 1 || n == 0 {
		...
		if out < 0 {
			panic("Переполнение n=" + strconv.Itoa(n))
		}
	}
	return
}

//FiboNumber Кэшированная регрессия с int64 (падает на 93)
...
//FiboNumberBig Кэшированная регрессия с big.Int
